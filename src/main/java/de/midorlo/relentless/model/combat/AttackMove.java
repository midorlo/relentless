...
public class AttackMove implements IAttackModifier {
    ...
    public Attack modify(Attack attack) {
        ...
        if (((attack.getAttackMove().getAttackType().equals(AttackType.Blunt)
                || (attack.getAttackMove().getAttackType().equals(AttackType.Pierce)))
                || (attack.getAttackMove().getAttackType().equals(AttackType.Special)))
                && attack.getBehemothPart().isWounded()) {
            bonus.setPartDamageMult(0.25d);
        }
        if (attack.getAttackMove().getAttackType().equals(AttackType.Slash)
                && attack.getBehemothPart().isWounded()) {
        ...
        if (attack.getAttackMove().getAttackType().equals(AttackType.Pierce)
                && (attack.getBehemothPart().getType().equals(BehemothPartType.Head)
                || attack.getBehemothPart().getType().equals(BehemothPartType.Horn))) {
        ...
        protected void initDamageBaseline() {
            ...
            switch (move.getAttackType()) {
                ...
                case Pierce:
                    ...
                    break;
                default:
                    log.warning(move.getAttackType().toString());
            }
